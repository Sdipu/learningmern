...
routes.post('/register',upload,(req,res,next)=>{
  ...
  var{name,email,password,confirmpassword,phoneno,adress} = req.body
//    var{image}=req.file.filename
//    const file= await sharp(req.file.buffer).resize({width:250,height:250}).png().toBuffer()
    //   var file =req.file.filename
  var err
  if(!name || !password|| !email || !confirmpassword || !adress || !phoneno){
...
//find model
var arr =[];
user.find(function(err,fnduser){
    console.log(fnduser);
    if(err) console.log(err)
    else{
  arr.push(fnduser.name);
  console.log(arr);

  //for (var ab in arr) console.log(ab.[0].name);
}
})
